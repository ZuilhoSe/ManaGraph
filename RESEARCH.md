...
- Universe \(C\) (Oracle catalog), inventory \(I \subseteq C\), commander with identity \(\chi\)
- Choose \(D \subset C\), \(|D| = 99\), singleton, identity \(\subseteq \chi\), format-legal
- Budget (hard): per-card cap \(p_c \le P_{\max}\), deck cap \(\sum p_c \le B\); owned copies may have \(p_c = 0\)
...
1. **Per-card cap** \(P_{\max}\): filter \(C\) *before* retrieval. This is a restricted universe (same move as inventory-only). Example: no card over \$5.
2. **Deck cap** \(B\): \(\sum_{c \in D} p_c \le B\). This is a knapsack on top of the 99-card, identity, and role constraints. Greedy fill can respect it; ILP encodes it as one linear inequality.
...
Budget also **helps discovery**. Staples are expensive; a tight \(P_{\max}\) or \(B\) is an anti-meta prior without touching EDHREC. Evaluate that explicitly: same commander, unconstrained vs \(P_{\max} = \$2\) vs \(B = \$50\), report novelty and role coverage — not whether the cheap deck looks like the EDHREC average.

Do **not** put prices in the embedding. Cost is not semantics. Keep \(p_c\) as metadata on the card row and as an ILP coefficient.
...
## How the academic profile becomes the contribution
    ...
    - theme as an **outbreak**; \(R_{\mathrm{eff}}\) of an engine
    ...
    - Validator: size, commander legality, banned list (`legalities` is stored and unused), singleton, identity, basic-land exceptions, **\(p_c \le P_{\max}\), \(\sum p_c \le B\)**
    ...
    **Fill:** from a commander + intent, retrieve a candidate pool \(P\) (\(|P| \approx 150\)–300), then select 99 by beam search or ILP (PuLP/OR-Tools) with hard constraints (including \(P_{\max}\) and \(B\)) and a linear synergy score.

    **Cut:** given ~110 names, drop extras by redundancy (embedding nearest-neighbor density) + curve penalty + role quotas (ramp, draw, interaction, wincon, lands). If over budget, cut the worst synergy-per-dollar (or the most redundant expensive card) until \(\sum p_c \le B\).
...
6. budget ablation: unconstrained vs \(P_{\max}\) vs deck cap \(B\) — legality and role coverage must still hold; spend must respect the snapshot
...
- **Hard:** legal 99, identity, singleton, owned-only / restricted-universe when requested, **\(p_c \le P_{\max}\) and \(\sum p_c \le B\)** on a frozen price snapshot
- **Budget quality:** leftover slack \(B - \sum p_c\) is not a goal; violating \(B\) is a fail. Optional later: Pareto of geometric score vs spend
